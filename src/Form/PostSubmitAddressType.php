...
class PostSubmitAddressType extends AbstractType {
	...
	public function handleCity(FormBuilderInterface $builder) {
		...
		$builder->get('country')->addEventListener(
			...
			function (FormEvent $event) use ($cityFormModifier) {
				...
				$cityFormModifier($event->getForm()->getParent(), $data);
			}
		);
	}
	...
	public function handleStreet(FormBuilderInterface $builder) {
		...
		// by uncommenting you will get "The child with the name "city" does not exist."
		/*$builder->get('city')->addEventListener(
			FormEvents::POST_SUBMIT,
			function (FormEvent $event) use ($streetFormModifier) {
				$data = $event->getForm()->getData();
				$streetFormModifier($event->getForm()->getParent(), null, $data);
			}
		);*/
	}
}